...
class Mlp(nn.Module):
    def __init__(self, network_detection, in_features, hidden_features=None, out_features=None):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.network_detection = network_detection
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Conv2d):
            gain = (8 * self.network_detection) ** (-1/4)
...
class Att(nn.Module):
    def __init__(self, network_detection, dim, num_heads, window_size, shift_size, use_attn=False, conv_type=None):
        super().__init__()
        self.dim = dim
...
class TB(nn.Module):
    def __init__(self, network_detection, dim, num_heads, mlp_ratio=4.,
        ...
        self.attn = Att(network_detection, dim, num_heads=num_heads, window_size=window_size,
                              shift_size=shift_size, use_attn=use_attn, conv_type=conv_type)

        self.norm2 = norm_layer(dim) if use_attn and mlp_norm else nn.Identity()
        self.mlp = Mlp(network_detection, dim, hidden_features=int(dim * mlp_ratio))
...
class LEGM(nn.Module):
    def __init__(self, network_detection, dim, detection, num_heads, mlp_ratio=4.,
        ...
        attn_detection = attn_ratio * detection

        if attn_loc == 'last':
            use_attns = [i >= detection-attn_detection for i in range(detection)]
        elif attn_loc == 'first':
            use_attns = [i < attn_detection for i in range(detection)]
        elif attn_loc == 'middle':
            use_attns = [i >= (detection-attn_detection)//2 and i < (detection+attn_detection)//2 for i in range(detection)]

        # build blocks
        self.blocks = nn.ModuleList([
            TB(network_detection=network_detection,
                             dim=dim,
                             num_heads=num_heads,
                             mlp_ratio=mlp_ratio,
...
class SKConv(nn.Module):
    ...
    def forward(self, x):
        # 前向传播函数
        # print(f"Input shape to SKConv: {x.shape}")
        feas = []
...
class DetectionLoss(nn.Module):
    ...
    def forward(self, batch_mask, s1):
        return torch.mean(torch.abs(batch_mask - s1))

class DIACMPN(nn.Module):
    def __init__(self, in_chans=3, out_chans=3, window_size=8,
                 embed_dims=[24, 48, 96, 48, 24],
                 mlp_ratios=[2., 4., 4., 2., 2.],
                 detections=[16, 16, 16, 8, 8],
                 num_heads=[2, 4, 6, 1, 1],
                 attn_ratio=[1/4, 1/2, 3/4, 0, 0],
                 conv_type=['DWConv', 'DWConv', 'DWConv', 'DWConv', 'DWConv'],
        ...
        # backbone
        self.legm1 = LEGM(network_detection=sum(detections), dim=embed_dims[0], detection=detections[0],
        ...
        self.legm2 = LEGM(network_detection=sum(detections), dim=embed_dims[1], detection=detections[1],
        ...
        self.legm3 = LEGM(network_detection=sum(detections), dim=embed_dims[2], detection=detections[2],
        ...
        # TODO
        self.skConv = SKConv(in_ch=168, M=3, G=1, r=4)

        assert embed_dims[1] == embed_dims[3]

        # TODO
        # self.mfm1 = MFM(embed_dims[3])
        self.mfm1 = DF_Module(embed_dims[3], embed_dims[3], True)

        self.legm4 = LEGM(network_detection=sum(detections), dim=embed_dims[3], detection=detections[3],
        ...
        self.mfm2 = DF_Module(embed_dims[4], embed_dims[4], True)

        self.legm5 = LEGM(network_detection=sum(detections), dim=embed_dims[4], detection=detections[4],
    ...
    def forward_features(self, x):
        ...
        # TODO
        fuse_feature = self.skConv(fuse_feature)
        ...
        # TODO
        test01 = self.skip2(skip2)
        x = self.mfm1(x, test01) + x
        ...
        # TODO
        test02 = self.skip1(skip1)
        x = self.mfm2(x, test02) + x
