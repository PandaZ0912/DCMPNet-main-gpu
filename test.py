...
def test_dehighlight(test_loader, network, dehighlight_result_dir):
	PSNR = AverageMeter()
	SSIM = AverageMeter()
	...
	os.makedirs(os.path.join(dehighlight_result_dir, 'imgs'), exist_ok=True)
	f_result = open(os.path.join(dehighlight_result_dir, 'dehighlight_results.csv'), 'w')
	...
	for idx, batch in enumerate(test_loader):
		...
		write_img(os.path.join(dehighlight_result_dir, 'imgs', filename), out_img)

	f_result.close()

	os.rename(os.path.join(dehighlight_result_dir, 'dehighlight_results.csv'),
			  os.path.join(dehighlight_result_dir, '%.02f | %.04f.csv'%(PSNR.avg, SSIM.avg)))
...
if __name__ == '__main__':
	...
	dehighlight_result_dir = os.path.join(args.dehighlight_result_dir, args.dataset, args.model)
	test_dehighlight(test_loader, network, dehighlight_result_dir)